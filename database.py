...
def init_db():
    ...
    with get_connection() as conn:
        with conn.cursor() as cursor:
            ...
            """)
            conn.commit()
    print("✅ Облачная база данных PostgreSQL успешно подключена!")
...
def get_or_create_user(user_id: int, username: str = ""):
    ...
    with get_connection() as conn:
        with conn.cursor() as cursor:
            ...
            if not row:
                cursor.execute(
                    "INSERT INTO users (user_id, username, balance, tickets) VALUES (%s, %s, 0.0, 0)",
                    (user_id, clean_user)
                )
                conn.commit()
            ...
            return {"user_id": row["user_id"], "username": f"@{row['username']}", "balance": float(row["balance"]), "tickets": row["tickets"]}
    ...
    with get_connection() as conn:
        with conn.cursor() as cursor:
            ...
            if clean_query.isdigit():
                ...
                row = cursor.fetchone()
                if row:
                    return {"user_id": row["user_id"], "username": f"@{row['username']}", "balance": float(row["balance"]), "tickets": row["tickets"], "created_at": row["created_at"]}
