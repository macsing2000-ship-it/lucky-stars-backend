...
CRYPTOBOT_TOKEN = os.getenv("CRYPTOBOT_TOKEN", "").strip() # Токен от @CryptoBot (Pay API)
...
async def test_win_handler(message: types.Message):
    """Секретная команда для проверки авто-выдачи звёзд через StarsFlow"""
    if not is_admin(message.from_user.id):
        return
    ...
    success = await send_stars_via_starsflow(username, amount)
    if success:
        await message.answer(f"✅ <b>Тест успешен!</b> StarsFlow принял заказ на {amount} ⭐ и должен отправить их пользователю.")
    else:
        await message.answer(f"❌ <b>Тест не удался!</b> Ошибка при запросе к StarsFlow (проверьте логи Render и правильность STARSFLOW_TOKEN).", parse_mode="HTML")
...
async def user_info_handler(message: types.Message):
    ...
    if not target:
        await message.answer("❌ Пользователь не найден в базе данных. (Пусть пользователь нажмет /start в боте)", parse_mode="HTML")
        return
...
async def give_balance_handler(message: types.Message):
    ...
    target = database.find_user(parts[1])
    if not target:
        await message.answer("❌ Пользователь не найден в базе. (Если у пользователя нет username, пополняйте по его цифровому ID)", parse_mode="HTML")
        return
...
async def create_cryptobot_invoice(req: CryptoBotRequest):
    """Создание счета через @CryptoBot (Pay API)"""
    if not CRYPTOBOT_TOKEN:
        raise HTTPException(status_code=500, detail="CryptoBot не настроен на сервере (нет CRYPTOBOT_TOKEN)")
    
    if req.amount_rub < 50:
        raise HTTPException(status_code=400, detail="Минимальная сумма пополнения через CryptoBot — 50 ₽")

    # Конвертируем рубли в USD по примерному курсу для CryptoBot (или запрашиваем в RUB, если касса поддерживает)
    # CryptoBot поддерживает оплату в RUB напрямую в некоторых версиях API
    ...
    # Конвертируем по курсу ~100 руб за 1 USDT (или передаем в RUB)
    # Для надежности используем RUB
    payload = {
        "asset": "USDT",
        "amount": str(round(req.amount_rub / 95.0, 2)), # Примерный расчет в USDT
        "description": f"Пополнение баланса Lucky Buy на {req.amount_rub} ₽",
        "payload": str(req.user_id),
        "paid_btn_name": "openBot",
        "paid_btn_url": f"https://t.me/{bot.get_me().username}" if bot else "https://t.me"
    }
    ...
    try:
        async with httpx.AsyncClient() as client:
            ...
            if data.get("ok"):
                ...
                raise HTTPException(status_code=400, detail="Ошибка создания счета в CryptoBot")
    except Exception as e:
