...
STARS_DEPOSIT_RATE = 1.50   # Зачисление рублей за 1 ⭐ пополнения (1 звезда = 1.5 ₽)
...
async def send_stars_via_starsflow(recipient: str, amount: int) -> tuple:
    """Диагностическая отправка звезд через StarsFlow с полным перебором путей"""
    ...
    # Полный диагностический список всех возможных эндпоинтов StarsFlow
    endpoints = [
        "https://tgstars.tg/api/v1/orders",
        "https://tgstars.tg/api/v1/order",
        "https://tgstars.tg/api/client/orders",
        "https://tgstars.tg/api/client/order",
        "https://tgstars.tg/api/v1/client/orders",
        "https://tgstars.tg/api/v1/client/order",
        "https://api.tgstars.tg/v1/orders",
        "https://api.tgstars.tg/v1/order",
        "https://api.tgstars.tg/api/v1/orders",
        "https://tgstars.tg/api/v1/orders/create"
    ]
    ...
    async with httpx.AsyncClient() as client:
        for url in endpoints:
            try:
                ...
                results_log.append(f"🌐 `{url}` ➔ Код {status}")

                # Если получили успешный ответ (200 OK или 201 Created)
                if status in (200, 201):
                    logger.info(f"✅ [StarsFlow] Успех на адресе {url}")
                    return True, f"Успешно отправлено через {url}"
                
                # Если адрес верный, но ошибка в балансе или токене (не 404)
                elif status in (400, 401, 403, 422):
                    logger.error(f"❌ [StarsFlow {url}] Ошибка {status}: {response.text}")
                    return False, f"Код {status} на {url}\nОтвет: {response.text}"

            except Exception as e:
                results_log.append(f"🌐 `{url}` ➔ Ошибка сети: {str(e)}")

    # Если все эндпоинты вернули ошибку (например, 404)
    details = "\n".join(results_log)
...
async def admin_panel(message: types.Message):
    ...
    text = (
        ...
        "🧪 <code>/testwin количество</code> — тест авто-выдачи звёзд"
    )
...
async def test_win_handler(message: types.Message):
    ...
    await message.answer(f"⏳ Тестируем отправку {amount} ⭐ пользователю @{username} через все эндпоинты StarsFlow...")
    ...
    if success:
        ...
        await message.answer(
            f"❌ <b>Тест не удался! Лог обхода эндпоинтов:</b>\n\n"
            f"{details}\n\n"
            f"💡 <i>Если один из адресов выдал код 401 — обновите токен в @StFlow_bot.\n"
            f"Если код 400 — не хватает баланса на StarsFlow.</i>",
            parse_mode="HTML"
        )
...
async def give_balance_handler(message: types.Message):
    ...
    parts = message.text.split()
    if len(parts) < 3:
        await message.answer("⚠️ Формат: <code>/give @username сумма</code> или <code>/give ID сумма</code>", parse_mode="HTML")
        return
...
async def successful_payment(message: types.Message):
    ...
    # Зачисляем баланс по курсу: 1 звезда = 1.50 рубля
...
# ----------------- ВЕБХУК ДЛЯ CRYPTOBOT (АВТОМАТИЧЕСКОЕ ЗАЧИСЛЕНИЕ) -----------------
    ...
    try:
        ...
        if CRYPTOBOT_TOKEN and signature:
            ...
            if not hmac.compare_digest(calculated_sig, signature):
                logger.error("🛑 Неверная подпись CryptoBot вебхука!")
        ...
        if update_type == "invoice_paid":
            ...
            if user_id_str and user_id_str.isdigit():
                ...
                logger.info(f"💎 [CryptoBot] Пользователь {user_id} оплатил счёт. Зачислено: +{rub_to_add} ₽")
                ...
                if bot:
                    try:
                        await bot.send_message(
                            ...
                            f"Сумма зачисления: <b>+{rub_to_add:.2f} ₽</b>\n"
                            f"Текущий баланс в игре: <b>{user_info['balance']:.2f} ₽</b>",
                            parse_mode="HTML"
                        )
                    except Exception as e:
        ...
        logger.error(f"Ошибка обработки CryptoBot вебхука: {e}")
        return {"ok": True}


# ----------------- API: СОЗДАНИЕ СЧЕТОВ (INVOICE) -----------------
...
async def create_cryptobot_invoice(req: CryptoBotRequest):
    if not CRYPTOBOT_TOKEN:
        raise HTTPException(status_code=500, detail="CryptoBot не настроен на сервере (нет CRYPTOBOT_TOKEN)")
    ...
    payload = {
        ...
        "description": f"Пополнение баланса Lucky Buy на {req.amount_rub:.0f} ₽",
    ...
    try:
        async with httpx.AsyncClient() as client:
            ...
            if data.get("ok"):
                ...
                return {"pay_url": result.get("pay_url")}
            else:
                logger.error(f"CryptoBot error: {data}")
                raise HTTPException(status_code=400, detail="Ошибка API CryptoBot")
    except HTTPException as he:
        raise he
    except Exception as e:
        logger.error(f"Ошибка запроса к CryptoBot: {e}")
        raise HTTPException(status_code=500, detail="Не удалось связаться с CryptoBot API")
...
async def make_spin(req: SpinRequest):
    ...
    if is_win:
        stars_won = req.stars
        auto_sent, _ = await send_stars_via_starsflow(req.recipient, stars_won)
    else:
        stars_won = 0
        tickets_won = 1
