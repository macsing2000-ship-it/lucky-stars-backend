...
# Настройка подробного логирования для отслеживания всех процессов
logging.basicConfig(
    level=logging.INFO,
...
# ----------------- ФУНКЦИЯ АВТО-ОТПРАВКИ ЗВЁЗД (STARSFLOW API) -----------------

async def send_stars_via_starsflow(recipient: str, amount: int) -> bool:
    """
    Отправляет выигранные звёзды напрямую на баланс пользователя Telegram через StarsFlow.
    Списывает звёзды с вашего франшизного баланса.
    """
    if not STARSFLOW_TOKEN:
        logger.error("[StarsFlow] Ошибка: STARSFLOW_TOKEN не задан в настройках Render!")
        return False

    clean_username = recipient.replace("@", "").strip()
    if not clean_username:
        logger.error("[StarsFlow] Ошибка: Передан пустой юзернейм для авто-выплаты.")
        return False
    ...
    try:
        async with httpx.AsyncClient() as client:
            ...
            if response.status_code == 200:
                ...
                logger.error(f"❌ [StarsFlow API Error] Статус {response.status_code}: {response.text}")
                return False
    except Exception as e:
        logger.error(f"❌ [StarsFlow Connection Error] Не удалось связаться с API: {e}")
        return False
...
async def give_balance_handler(message: types.Message):
    ...
    try:
        ...
        logger.warning(f"Не удалось отправить личное сообщение пользователю {target['user_id']}: {e}")
...
async def pre_checkout_handler(pre_checkout_query: PreCheckoutQuery):
    """
    Обязательный шаг для Telegram-платежей.
    Сервер должен моментально ответить Telegram 'ОК' в течение 10 секунд,
    иначе транзакция отменится с ошибкой таймаута.
    """
...
async def successful_payment_handler(message: types.Message):
    """
    Вызывается автоматически ПОСЛЕ того, как пользователь оплатил звёзды.
    Начисляет эквивалент рублей на баланс игрока в нашей базе данных.
    """
    ...
    try:
        ...
        # Переводим звезды в рубли (курс: 1 звезда = 1.95 ₽)
        ...
        logger.info(f"💰 Баланс пользователя {user_id} успешно пополнен на {rub_amount} руб. через Telegram Stars")
    except Exception as e:
        logger.error(f"❌ Ошибка зачисления платежа Stars: {e}")
...
# ----------------- API ДЛЯ ВЕБ-ПРИЛОЖЕНИЯ (FASTAPI) -----------------
...
async def create_invoice(req: InvoiceRequest):
    """
    Генерирует счет (инвойс) на оплату звёзд прямо в Telegram.
    XTR — это официальное международное обозначение валюты Telegram Stars.
    """
    try:
        ...
        link = await bot.create_invoice_link(
            ...
            payload=payload,
            provider_token="",  # Пустое поле для цифровых товаров / Telegram Stars
...
async def make_spin(req: SpinRequest):
    ...
    Рассчитывает стоимость игры, списывает баланс, разыгрывает шанс 
    и автоматически высылает выигрыш победителю через StarsFlow API.
    """
    ...
    # Списание стоимости броска из баланса в базе данных
    ...
    if is_win:
        stars_won = req.stars
        # Мгновенная автоматическая отправка выигранных звезд через StarsFlow API
    ...
    # Логирование раунда игры в базу данных
    database.log_game(req.user_id, req.username, req.stars, req.chance, cost, is_win, req.recipient, tickets_won)
    
    # Уведомление администратора в случае выигрыша звёзд (с подробным HTML оформлением)
    if is_win:
        status_str = "✅ Авто-доставка успешна!" if auto_sent else "⚠️ Внимание: Требуется ручная отправка (проверьте баланс StarsFlow)"
