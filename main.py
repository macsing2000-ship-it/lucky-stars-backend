...
# Ключи CrystalPay
CRYSTALPAY_LOGIN = os.getenv("PAYMENT_SHOP_ID", "").strip().lower()
CRYSTALPAY_SECRET = os.getenv("PAYMENT_SECRET_KEY", "").strip()
...
async def create_deposit(req: DepositRequest):
    ...
    if not CRYSTALPAY_LOGIN or not CRYSTALPAY_SECRET:
        logger.error("PAYMENT_SHOP_ID или PAYMENT_SECRET_KEY не настроены в Render Environment!")
    ...
    try:
        async with httpx.AsyncClient() as client:
            ...
            logger.info(f"CrystalPay Response: {data}")
            
            if data.get("error") == False or data.get("error") == "false":
                return {"url": data.get("url"), "id": data.get("id")}
            else:
                err_msg = ", ".join(data.get("errors", ["Неизвестная ошибка CrystalPay"]))
                raise HTTPException(status_code=400, detail=f"Ошибка CrystalPay: {err_msg}")
    except HTTPException as he:
        raise he
    except Exception as e:
        logger.error(f"Ошибка соединения с CrystalPay: {e}")
        raise HTTPException(status_code=500, detail="Ошибка соединения с сервером оплаты")
...
async def crystalpay_callback(request: Request):
    ...
    try:
        ...
        logger.info(f"Получен платежный вебхук: {data}")
        ...
        if bot:
            try:
                ...
                logger.error(f"Ошибка отправки уведомления игроку: {e}")

        return {"ok": True}
    except Exception as e:
        logger.error(f"Ошибка обработки коллбэка: {e}")
        return {"ok": True}
