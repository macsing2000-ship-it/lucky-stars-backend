...
async def send_stars_via_starsflow(recipient: str, amount: int) -> tuple:
    """Отправка звезд через StarsFlow. Возвращает (успех_bool, детали_str)"""
    ...
    try:
        async with httpx.AsyncClient() as client:
            ...
            if response.status_code == 200:
                ...
                logger.error(f"❌ [StarsFlow API Error]: {response.status_code} - {err_text}")
...
async def test_win_handler(message: types.Message):
    ...
    if success:
        ...
        await message.answer(
            ...
            f"<code>{details}</code>\n\n"
            f"💡 <i>Решение:\n"
            f"• Если код 401 — скопируйте НОВЫЙ токен по кнопке «Обновить токен» в @StFlow_bot.\n"
...
async def cryptobot_webhook(request: Request):
    try:
        ...
        # Проверка HMAC-SHA256 подписи CryptoBot
