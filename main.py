...
async def send_stars_via_starsflow(recipient: str, amount: int) -> tuple:
    """Отправка звезд через StarsFlow с перебором актуальных эндпоинтов API"""
        ...
        for url in endpoints:
            try:
                ...
                if response.status_code == 200 or response.status_code == 201:
                    logger.info(f"✅ [StarsFlow] Успешно отправлено {amount} ⭐ через {url}")
                    return True, f"Успешно ({url})"
                elif response.status_code != 404:
                    # Если статус не 404, значит URL правильный, но есть другая причина (например, баланс)
                    err_text = response.text
                    logger.error(f"❌ [StarsFlow API Error {url}]: {response.status_code} - {err_text}")
                    return False, f"Код {response.status_code}: {err_text}"
                else:
                    last_error = f"Код 404 на {url}"
            except Exception as e:
...
async def test_win_handler(message: types.Message):
    ...
    await message.answer(f"⏳ Тестируем отправку {amount} ⭐ пользователю @{username} через StarsFlow API...")
    
    success, details = await send_stars_via_starsflow(username, amount)
    if success:
        await message.answer(f"✅ <b>Тест успешен!</b> StarsFlow принял заказ на {amount} ⭐.\n<i>Детали: {details}</i>", parse_mode="HTML")
    else:
        await message.answer(
            f"❌ <b>Тест не удался!</b>\n\n"
            f"📝 <b>Ответ сервера StarsFlow:</b>\n"
            f"<code>{details}</code>\n\n"
            f"💡 <i>Если код 401 — скопируйте НОВЫЙ токен по кнопке «Обновить токен» в @StFlow_bot.\n"
            f"Если код 400 (нехватка средств) — докиньте на StarsFlow 5–10 рублей для запаса при округлении.</i>",
            parse_mode="HTML"
        )
