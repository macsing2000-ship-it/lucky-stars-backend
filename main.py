...
# КУРСЫ ВАЛЮТ ДЛЯ ПРИБЫЛИ
STAR_PRICE_RUB = 1.95      # Стоимость звезды в рулетке
STARS_DEPOSIT_RATE = 1.10  # Зачисление рублей за 1 ⭐ пополнения (учитывая комиссию Fragment)
...
async def successful_payment(message: types.Message):
    ...
    # Зачисляем баланс по выверенному курсу
...
async def create_cryptobot_invoice(req: CryptoBotRequest):
    ...
    usdt_amount = str(round(req.amount_rub / 95.0, 2))
    if float(usdt_amount) < 0.1:
        usdt_amount = "0.1"
