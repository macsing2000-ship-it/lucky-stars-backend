...
async def send_stars_via_starsflow(recipient: str, amount: int) -> bool:
    if not STARSFLOW_TOKEN:
        logger.error("[StarsFlow] STARSFLOW_TOKEN не задан!")
        return False

    clean_username = recipient.replace("@", "").strip()
    if not clean_username:
        logger.error("[StarsFlow] Передан пустой юзернейм!")
        return False
    ...
    try:
        ...
        logger.error(f"❌ Ошибка соединения со StarsFlow: {e}")
        return False
...
async def admin_panel(message: types.Message):
    ...
    text = (
        ...
        "🎟 <code>/tickets @username количество</code> — изменить билеты (+5 или -2)"
    )
...
async def make_spin(req: SpinRequest):
    ...
    if is_win:
        status_str = "✅ Авто-доставка успешна!" if auto_sent else "⚠️ Требуется ручная отправка"
