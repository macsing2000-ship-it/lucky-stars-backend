...
async def test_win_handler(message: types.Message):
    ...
    if success:
        ...
        await message.answer(f"❌ <b>Тест не удался!</b> Проверьте баланс StarsFlow и токен.", parse_mode="HTML")
...
async def cryptobot_webhook(request: Request):
    """Вебхук от @CryptoBot об успешной оплате"""
    try:
...
async def create_cryptobot_invoice(req: CryptoBotRequest):
    ...
    # Выставляем счёт строго в фиатных рублях (RUB), чтобы сумма не занижалась курсом
    payload = {
        "currency_type": "fiat",
        "fiat": "RUB",
